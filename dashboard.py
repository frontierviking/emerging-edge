...
const EXCHANGE_HOLIDAYS = __EXCHANGE_HOLIDAYS_JSON__;
// Markets that move a weekend holiday to the next business day.
const _IN_LIEU_MONDAY = __IN_LIEU_MONDAY_JSON__;
const _IN_LIEU_NEAREST = __IN_LIEU_NEAREST_JSON__;
...
function _baseHolidayName(exDate, holidayList) {
...
// Days in lieu: a holiday landing on a weekend closes the following
// Monday in most Commonwealth/Asian markets (and the adjacent weekday
// in the US). Singapore's National Day fell on Sunday 09 Aug 2026, so
// SGX was shut on Monday the 10th — without this the badge said "open"
// and every SGX price just sat flat with no explanation.
function _exchangeHolidayName(exDate, holidayList, country) {
    const direct = _baseHolidayName(exDate, holidayList);
    ...
    const dow = exDate.getDay();          // 0 = Sunday, 1 = Monday
    if (_IN_LIEU_MONDAY.indexOf(country) >= 0 && dow === 1) {
        for (const back of [1, 2]) {      // Sunday, then Saturday
            const nm = _baseHolidayName(shift(-back), holidayList);
            if (nm) return nm + ' (observed)';
        }
    }
    if (_IN_LIEU_NEAREST.indexOf(country) >= 0) {
        if (dow === 1) {
            const nm = _baseHolidayName(shift(-1), holidayList);
            if (nm) return nm + ' (observed)';
        }
        if (dow === 5) {                  // Friday for a Saturday holiday
            const nm = _baseHolidayName(shift(1), holidayList);
            if (nm) return nm + ' (observed)';
        }
    }
...
function _isExchangeHoliday(exDate, holidayList, country) {
    return _exchangeHolidayName(exDate, holidayList, country) !== null;
}
...
function getExchangeStatus(exCode) {
    ...
    const holidayName = _exchangeHolidayName(exTime, EXCHANGE_HOLIDAYS[exCode], exCode);
...
# The holiday table is generated from market_calendar.EXCHANGE_HOLIDAYS
# (Python) rather than hand-maintained here, so the open/closed badge
# and the server's "is this market even trading?" skip always agree.
try:
    import json as _json_hol
    import market_calendar as _mc_hol
    JS = JS.replace("__EXCHANGE_HOLIDAYS_JSON__",
                    _json_hol.dumps(_mc_hol.EXCHANGE_HOLIDAYS,
                                    ensure_ascii=False, sort_keys=True))
    JS = JS.replace("__IN_LIEU_MONDAY_JSON__",
                    _json_hol.dumps(sorted(_mc_hol._IN_LIEU_NEXT_MONDAY)))
    JS = JS.replace("__IN_LIEU_NEAREST_JSON__",
                    _json_hol.dumps(sorted(_mc_hol._IN_LIEU_NEAREST_WEEKDAY)))
except Exception as _e:      # never break the page over the badge
    JS = JS.replace("__EXCHANGE_HOLIDAYS_JSON__", "{}")
    JS = JS.replace("__IN_LIEU_MONDAY_JSON__", "[]")
    JS = JS.replace("__IN_LIEU_NEAREST_JSON__", "[]")
